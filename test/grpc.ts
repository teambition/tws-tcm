...
suite('gRPC Client', function (this: Suite) {
  this.timeout(10000)
  ...
  const client = new TCMClient({
    ...
    timeout: 5000,
    mode: 'grpc',
    rootCert: readFileSync('./certs/ca.cert'),
    privateKey: readFileSync('./certs/server.key'),
    certChain: readFileSync('./certs/server.crt'),
  })
